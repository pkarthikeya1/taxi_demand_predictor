...
def validate_raw_data(
    ...
    # remove rows with pickup_datetime outside their valid range
...
def load_raw_data(
    ...
    """ Loads raw data from local storage or downloads it from the NYC
      website, and the loads it into a Dataframe"""
    rides = pd.DataFrame()

    if months is None:
        # download data  for the entire year
    ...
    for month in months:
        ...
        if not local_file.exists():
            ...
            # download the file from the NYC website
                ...
                print(f"Dowloading file{year}-{month:02d} file is not available")
                continue
        else:
...
def add_missing_slots(agg_rides:pd.DataFrame) -> pd.DataFrame :
    ...
    full_range =pd.date_range(
        start=agg_rides['pickup_hour'].min(), end=agg_rides['pickup_hour'].max(), freq='h')
    output=pd.DataFrame()
...
def transform_ts_data_into_features_and_target(
    ...
    input_seq_len: int,
    step_size: int ) -> Tuple[pd.DataFrame, pd.Series]:
    """
